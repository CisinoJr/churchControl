...
@Component({
    selector: 'jhi-endereco-church',
    templateUrl: './endereco-church.component.html'
})
export class EnderecoChurchComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/endereco-church'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/endereco-church', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: EnderecoChurch) {
        return item.id;
    }
